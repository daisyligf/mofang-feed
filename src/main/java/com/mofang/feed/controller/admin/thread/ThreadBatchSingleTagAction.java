 ...
 *
 */
@Action(url = "backend/threads/single_tag")
public class ThreadBatchSingleTagAction extends AbstractActionExecutor
{
	...
	protected ResultValue exec(HttpRequestContext context) throws Exception
	{
		...
		for(String strThreadId : arrThreadIds)
		{
			if(tagId == ThreadTag.ELITE)
				result = logic.setElite(Long.parseLong(strThreadId), operatorId, reason);
			else if(tagId == ThreadTag.MARK)
				result = logic.setMark(Long.parseLong(strThreadId), operatorId, reason);
