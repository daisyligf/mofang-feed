 ...
 *
 */
@Action(url = "backend/comment/restoreRecoverComments")
public class CommentBatchRestoreAction extends AbstractActionExecutor
{
	...
	protected ResultValue exec(HttpRequestContext context) throws Exception
	{
		...
		///参数检查
		if(StringUtil.isInteger(commentIds) || operatorId <= 0)
		{
			result.setCode(ReturnCode.CLIENT_REQUEST_DATA_IS_INVALID);
			result.setMessage(ReturnMessage.CLIENT_REQUEST_DATA_IS_INVALID);
			return result;
		...
		{
			result = logic.restore(Long.parseLong(strCommentId), operatorId);
