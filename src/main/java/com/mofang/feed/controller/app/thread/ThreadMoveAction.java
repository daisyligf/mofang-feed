 ...
 *
 */
@Action(url = "feed/v2/threads/move")
public class ThreadMoveAction extends AbstractActionExecutor
{
	...
	protected ResultValue exec(HttpRequestContext context) throws Exception
	{
		...
		long destForumId = json.optLong("target_fid", 0L);
		String threadIds = json.optString("tids", "");
		String reason = json.optString("reason", "");
		
		///参数检查
		if(StringUtil.isNullOrEmpty(threadIds) || destForumId <= 0)
		{
			result.setCode(ReturnCode.CLIENT_REQUEST_DATA_IS_INVALID);
			result.setMessage(ReturnMessage.CLIENT_REQUEST_DATA_IS_INVALID);
			return result;
		...
		{
			result = logic.move(Long.parseLong(strThreadId), operatorId, reason, destForumId);
